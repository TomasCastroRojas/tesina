...
Lemma one_step_remote_services_preserves_valid_attacker_ii : forall (a a' : Attacker) (n: network_map) (aValid: valid_attacker a n) (m m': idMachine) (u u': idUser) (k': option key) (s': idService),
      valid_concrete_network n -> Pre a (Remote_Services m u m' u' k' s') -> Post a (Remote_Services m u m' u' k' s') n a' -> valid_attacker_ii a'.
  Proof.
    intros a a' network validAttacker m m' u u' k' s' validNetwork pre post.
    unfold valid_attacker_ii.
    intros m0 u0 secrets_a'.
    ...
    elim H2'; intros env Hmastered; clear H2'.
    unfold valid_attacker_ii in validAttackerII.
    rewrite env.
    apply validAttackerII.
    rewrite secrets in secrets_a'.
    exact secrets_a'.
  Qed.